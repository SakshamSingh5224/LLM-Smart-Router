...
def extract_signals(query: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "has_code": bool(any(re.search(pat, query, re.IGNORECASE) for pat in CODE_PATTERNS)),
        "has_math": bool(any(re.search(pat, query, re.IGNORECASE) for pat in MATH_PATTERNS)),
        "has_reasoning": bool(any(re.search(pat, q_lower) for pat in REASONING_PATTERNS)),
